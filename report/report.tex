...
\begin{category}[L]{List is partially sorted}
  \textbf{Input:} \texttt{list = [$x_0, x_1, \dots, x_n$]} for $n > 1$, there exists some $i, j$
  such that for all $i \geq 0,\  i < j,\  j \leq n$, and \texttt{[$x_i, \dots, x_j$].length $\neq
  n$}, the sub-list $x_i, \dots, x_j$ is sorted.
  \par
  \textbf{Reason:} A derivative of \textbf{Category L4} to test the stability of the
  \texttt{Collections.sort} sorting algorithm. The sorting algorithm should sort the partially
  sorted elements in the same order that they appear in the input.
\end{category}
